...
object Monads extends App {
  ...
  /*
      ...
      2) Monads = unit + flatMap
         ...
         Monad [T] {
          def flatMap[B](f: T=> Monad([B]): Monda[B] = ... (implemented)
          def map[B](f: T => B): Monad[B] = ???
          def flatted(m: Monad[Monad[T]]): Monad[T] = ???
  ...
  /*
      Monad[T] {
        def flatMap[B](f: T=> Monad([B]): Monda[B] = ... (implemented)
        def map[B](f: T => B): Monad[B] = flatMap(x => unit(f(x)) // End result is Monad[B]
        def flatted(m: Monad[Monad[T]]): Monad[T] = m.flatMap((x: Monad[T]) => x
