...
object ThreadCommunication extends App {
  ...
  def smartProdCon(): Unit = {
    ...
    val producer = new Thread(() => {
      ...
      container.synchronized {
        ...
        //It will notify the consumer to start consuming
        container.notify()
      }
    })
  ...
  def testNotifyAll(): Unit = {
    ...
    new Thread(() => {
      ...
      bell.synchronized {
        //        bell.notify() // If only notify only 1 thread know and hence the rest of the thread are still waiting
        bell.notifyAll()
      }
    }).start()
  }
  ...
  case class Friend(name: String) {
    ...
    def pass(other: Friend): Unit = {
      ...
      println(s"$other at $this.side is passing the $other.side")
    }
  }
